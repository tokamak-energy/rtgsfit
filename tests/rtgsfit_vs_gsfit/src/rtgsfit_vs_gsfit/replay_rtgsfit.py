...
def replay_rtgsfit(time: float):
    ...
    meas_pcs = rtgsfit_signalprep.prep_meas(time)
    ...
    with mdsthin.Connection('smaug') as conn:
        ...
        psi = conn.get(f"\\GSFIT::TOP.{cnst.RUN_NAME}.TWO_D.PSI").data()
        psi_a = conn.get(f"\\GSFIT::TOP.{cnst.RUN_NAME}.GLOBAL:PSI_A").data()[0]
        psi_b = conn.get(f"\\GSFIT::TOP.{cnst.RUN_NAME}.GLOBAL:PSI_B").data()[0]
        mask = conn.get(f"\\GSFIT::TOP.{cnst.RUN_NAME}.TWO_D:MASK").data()
    ...
    output_dict = {"meas_pcs" : np.zeros((cnst.N_ITERS + 1, n_meas_pcs), dtype=np.float64),
