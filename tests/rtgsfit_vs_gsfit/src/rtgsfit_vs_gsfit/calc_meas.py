"""
This module contains a functions to calculate the predicted measurements from RTGSFIT at
a given iteration.
"""
...
def calculate_predicted_measurements(flux_norm: np.ndarray,
                                     coef: np.ndarray,
    ...
    with mdsthin.Connection('smaug') as conn:
        ...
        n_grid = conn.get(f"\\RTGSFIT::TOP.{cnst.RUN_NAME}.PRESHOT:N_GRID").data()
        ...
        g_grid_meas_weight = conn.get(f"\\RTGSFIT::TOP.{cnst.RUN_NAME}.PRESHOT.GREENS:GRID_MEAS_W").data()
        g_coef_meas_weight = conn.get(f"\\RTGSFIT::TOP.{cnst.RUN_NAME}.PRESHOT.GREENS:COEF_MEAS_W").data()
        g_meas_coil = conn.get(f"\\RTGSFIT::TOP.{cnst.RUN_NAME}.PRESHOT.GREENS:MEAS_COIL").data()
        weight = conn.get(f"\\RTGSFIT::TOP.{cnst.RUN_NAME}.PRESHOT:WEIGHT").data()
        r_flat = conn.get(f"\\RTGSFIT::TOP.{cnst.RUN_NAME}.PRESHOT:R_GRID").data()
    ...
    g_pls_grid[1, :] = (1 - flux_norm)  / (r_flat * cnst.MU_0)
    ...
    pred_meas = pred_meas / (weight + (weight == 0))  # Avoid division by zero
...
if __name__ == "__main__":
    n_r = 33
    n_z = 65
    flux_norm = np.zeros(n_r * n_z, dtype=np.float64)
    n_coef = 25
    coef = np.zeros(n_coef, dtype=np.float64)
    n_coil = 8
    coil_curr = np.zeros(n_coil, dtype=np.float64)
    pred_meas = calculate_predicted_measurements(flux_norm, coef, coil_curr)