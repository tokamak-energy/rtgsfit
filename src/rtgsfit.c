        ...
        )
{
    ...
    // FILE *file = fopen("basis_output.txt", "w");
        ...
        )
{
    ...
    // // Write g_coef_meas_w to a file
    // FILE *file = fopen("g_coef_meas_w.txt", "w");
    // for (int i = 0; i < N_COEF * N_MEAS; i++) {
    //     fprintf(file, "%.15lf\n", g_coef_meas_w[i]);
    // }
    // fclose(file);

    // FILE *file2 = fopen("g_coef_meas_w_with_regularisation.txt", "w");
    // for (int i = 0; i < N_COEF * (N_MEAS+5); i++) {
    //     fprintf(file2, "%.15lf\n", g_coef_meas_w_with_regularisation[i]);
    // }
    // fclose(file2);
    ...
    // // Print the values in "coef" to screen
    // printf("Values in coef (length %d):\n", N_COEF);
    ...
    *error = 0.0;
    ...
    // find x point & opt
    printf("about to call find_null_in_gradient_march\n");
    find_null_in_gradient_march(flux_total, opt_r, opt_z, opt_flux, &opt_n,
            xpt_r, xpt_z, xpt_flux, &xpt_n);

    // printf("xpt_r %lf, xpt_z %lf, xpt_flux %lf, xpt_n %d\n",  xpt_r[0], xpt_z[0], xpt_flux[0], xpt_n);
