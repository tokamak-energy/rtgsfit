        ...
        )
{
    ...
    *xpt_n = 0;
    ...
    for (int i_row=1; i_row<(N_Z-1); i_row++)
    {
        for (int i_col=1; i_col<(N_R-1); i_col++)
        {
        ...
        )
{
    ...
    if ((col_start < 0) || (col_start > N_R)) {
      printf("horrible error col_start\n");
    }
    if ((col_end < 0) || (col_end > N_R)) {
      printf("horrible error col_end\n");
    }
    if (col_end < col_start) {
      printf("horrible error col st col_end\n");
    }
