...
The flux of the plasma current on the grid is obtained from the LIUQE
eq. (45) finite-difference form of the Grad-Shafranov operator by the von
Hagenow method: one solve with zero boundary flux, the boundary flux from the
normal derivative and the boundary Green's matrix `G_LTRB`, and one solve with
that boundary flux (`src/poisson_solver.c`). The operator is supplied to the
real-time code as precomputed LU factors (`LOWER_BAND`, `UPPER_BAND`,
`PERM_IDX` in `constants.c`), which `src/solve_tria.c` applies by banded
substitution.

Since the vertical coupling of the stencil is the same on every row, the
system is separable: `src/poisson_fast.c` diagonalises the vertical direction
with a sine transform and solves one small tridiagonal system per vertical
mode, which gives the same flux as the banded substitution to rounding at a
fraction of the cost. The stencil coefficients are recovered from the LU
factors on the first call (`poisson_solver_init()`), every row of the operator
is checked to have the separable five-point structure, and the fast solver is
cross-checked against the banded substitution before it is used; if any check
fails, `poisson_solver()` keeps using `solve_tria()` and
`poisson_solver_method()` reports which path is active. Calling
`poisson_solver_init()` once at start-up keeps this one-off table
construction (a few milliseconds) out of the first real-time cycle.
...
The fast Poisson solver is tested against the banded substitution and against
the LIUQE stencil on the same ST40-shaped constants, and its fallback is tested
on a deliberately non-separable operator (`test_data/constants_poisson_fallback.c`,
generated by `test_data/make_constants_poisson_fallback.py`):
```bash
cd src/
cp ../test_data/constants.c .
make SHOT=0 RUN_NAME=no_mds DEBUG=1 test_poisson_fast test_poisson_fallback
./test_poisson_fast
./test_poisson_fallback
```

### 5.1.1 Timing benchmark without MDSplus

`tests/rtgsfit_bench.c` calls `rtgsfit()` repeatedly on a fixed set of
inputs, records the wall-clock time of every call (and the per-section timers
when built with `TIMING=1`), and dumps the outputs so two builds can be
compared. The analytic test can export a matching input set:
```bash
cd tests/rtgsfit_verify_analytic
source .venv/bin/activate
python -m rtgsfit_verify_analytic.export_bench_inputs      # writes data/bench/
cd ../../src
cp ../tests/rtgsfit_verify_analytic/data/constants.c .
make SHOT=0 RUN_NAME=no_mds DEBUG=0 TIMING=1
cd ../tests
make -f makefile_bench TIMING=1
OPENBLAS_NUM_THREADS=1 ./rtgsfit_bench rtgsfit_verify_analytic/data/bench 10 2000 /tmp/bench_out
```
