...
real-time code as its stencil coefficients `POISSON_A`, `POISSON_B` and
`POISSON_C` (one value per radial grid column, see `constants.h`), computed
before the shot by the same code that generates the Green's matrices.
...
mode. The transform is a dense matrix product (two `cblas_dgemm` calls per
direction), so any grid size works. `poisson_solver_init()` builds the
transform tables and the per-mode tridiagonal factors from the stencil (about
a millisecond at 8000 grid points), checks the pivots, and checks the solver
against the stencil residual; it runs on the first solve if it has not been
called at start-up, and returns a nonzero status if the stencil is unusable,
in which case the solves return zero flux and `rtgsfit()` reports
`ERR_NO_AXIS`.
...
The Poisson solver is tested on the same ST40-shaped constants against a
dense LAPACK solve of the same system and against the LIUQE stencil, and its
rejection of an unusable stencil is tested on a small fixture
(`test_data/constants_poisson_invalid.c`, generated by
`test_data/make_constants_poisson_invalid.py`):
```bash
cd src/
cp ../test_data/constants.c .
make SHOT=0 RUN_NAME=no_mds DEBUG=1 test_poisson_fast test_poisson_invalid
./test_poisson_fast
./test_poisson_invalid
```
